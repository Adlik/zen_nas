...
'''
# pylint: disable=W0613,invalid-name,too-many-branches
import os
import sys
import time
import argparse
...
class LinearRegionCount():
    ...
    def update1D(self, activationList):
        code_string = ''
        for _, value in activationList.items():
...
class Linear_Region_Collector:
    ...
    def __init__(self, models=None, input_size=(64, 3, 32, 32), gpu=None,
        ...
        self.seed = seed
        self.gpu = gpu
        self.device = torch.device(f'cuda:{self.gpu}') if self.gpu is not None else torch.device('cpu')
        self.LRCounts = None
    ...
    def reinit(self, models=None, input_size=None, sample_batch=None, seed=None):
        if models is not None:
            ...
            self.LRCounts = [LinearRegionCount(self.input_size[0] * self.sample_batch, gpu=self.gpu)
        ...
        if input_size is not None or sample_batch is not None:
            ...
            #                                      num_workers=16, pin_memory=True, drop_last=True, shuffle=True)
    ...
    def register_hook(self, model):
        for module in model.modules():
            if isinstance(module, nn.ReLU):
                module.register_forward_hook(hook=self.hook_in_forward)

    def hook_in_forward(self, module, input_, output):
        if isinstance(input_, tuple) and len(input_[0].size()) == 4:
...
def recal_bn(network, xloader, recalbn, device):
    for module in network.modules():
        if isinstance(module, torch.nn.BatchNorm2d):
            module.running_mean.data.fill_(0)
            module.running_var.data.fill_(0)
            module.num_batches_tracked.data.zero_()
...
def get_ntk_n(networks, recalbn=0, train_mode=False, num_batch=None,
            ...
            for _idx in range(len(inputs_)):
                ...
                for name, weight in network.named_parameters():
                    if 'weight' in name and weight.grad is not None:
                        grad.append(weight.grad.view(-1).detach())
    ...
    conds = []
    for ntk_ in ntks:
        eigenvalues, _ = torch.symeig(ntk_)  # ascending
