...
def smart_round(digit, base=None):
    """change digit to a multiple of base"""
    if base is None:
        if digit > 32 * 8:
            round_base = 32
        elif digit > 16 * 8:
    ...
    return max(round_base, round(digit / float(round_base)) * round_base)
...
def save_pyobj(filename, pyobj):
    ...
    with open(filename, 'w', encoding='utf8') as fid:
        fid.write(the_s)
...
def parse_cmd_options(argv):  # pylint: disable=too-many-statements
    ...
    parser.add_argument('--dist_mode', type=str, default='cpu', help='Distribution mode, \
                        could be cpu, single, horovod, apex.')
    parser.add_argument('--independent_training', action='store_true', help='When distributed training, \
                        use each gpu separately.')
    ...
    parser.add_argument('--epoch_offset', default=0.0, type=float, help='Make the learning rate decaying as \
                        epochs + epoch_offset but start from epoch_offset. ')
