...
from SearchSpace.search_choice import get_mutated_structure_str, get_random_initialized_structure_str, register_serach_space
...
def main(args, argv):
    ...
    if args.fix_initialize:
        if '--plainnet_struct_txt' in argv:
            ...
            logging.info('Warning: plainnet_struct_txt is None, so random method and default search space will be used to initilize population, \n \
                please make sure that the default search space ([SuperResKXKX, SuperResK1KXK1]) meet your expectation.')
    ...
    for loop_count in range(args.evolution_max_iter):
        ...
        if loop_count >= 1 and loop_count % 1000 == 0:
            ...
            logging.info('Computing model score has elapsed {} hours'.format(acc_time / 3600))
        ...
        if len(popu_structure_list) <= 10:
            ...
            if not args.origin:
                random_structure_str = get_mutated_structure_str(
                    AnyPlainNet=AnyPlainNet, structure_str=tmp_random_structure_str, num_classes=args.num_classes, num_replaces=2)
            else:
                random_structure_str = get_new_random_structure_str(
