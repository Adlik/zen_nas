...
"""
# pylint: disable=W0401, line-too-long,too-many-lines
import os
import sys
...
try:
    ...
    from efficientnet_builder import BN_EPS_TF_DEFAULT, round_channels, EfficientNetBuilder, initialize_weight_goog, \
        initialize_weight_default, decode_arch_def, resolve_act_layer, resolve_bn_args
except ImportError:
...
class GenEfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=3, num_features=1280, stem_size=32, fix_stem=False,
        ...
        for name, module in self.named_modules():
            if weight_init == 'goog':
                initialize_weight_goog(module, name)
            else:
                initialize_weight_default(module, name)
    ...
    def extract_stage_features_and_logit(self, feature, target_downsample_ratio=16):
        """extract stages according to downsample ratio and return stage features and logit"""
        stage_features_list = []
        image_size = feature.shape[2]
        feature = self.conv_stem(feature)
        feature = self.bn1(feature)
        feature = self.act1(feature)
        ...
        for block in self.blocks:
            ...
            dowsample_ratio = round(image_size / feature.shape[2])
            if dowsample_ratio == target_downsample_ratio:
                stage_features_list.append(feature)
    ...
    def forward(self, input_):
        ...
        input_ = self.global_pool(input_)
