...
def load_pretrained(model, url, filter_fn=None, strict=True):
    ...
    if in_chans != pretrained_in_chans:
        if in_chans == 1:
            print(f'=> Converting pretrained input conv {input_conv_weight} from {pretrained_in_chans} to 1 channel')
            conv1_weight = state_dict[input_conv_weight]
            state_dict[input_conv_weight] = conv1_weight.sum(dim=1, keepdim=True)
        else:
            print('=> Discarding pretrained input conv {input_conv_weight} \
                   since input channel count != {pretrained_in_chans}')
    ...
    if num_classes != pretrained_num_classes:
        print(f'=> Discarding pretrained classifier since num_classes != {pretrained_num_classes}')
