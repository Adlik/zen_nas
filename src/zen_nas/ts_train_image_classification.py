...
def init_model(model, opt, argv):
    if hasattr(opt, 'weight_init') and opt.weight_init == 'xavier':
        ...
        if not hasattr(model, 'init_parameters'):
            logger.info('Warning! No init_parameters found')
        else:
            model.init_parameters()
    elif hasattr(opt, 'weight_init') and opt.weight_init == 'None':
        logger.info('Warning!!! model loaded without initialization !')
    else:
...
def train_one_epoch(train_loader, model, criterion, optimizer, epoch, opt, num_train_samples, no_acc_eval=False):
    ...
    # if distributed, sync
    if opt.dist_mode == 'horovod' and (not opt.independent_training):
        sync_tensor = torch.tensor([top1.sum, top1.count, top5.sum, top5.count, losses.sum, losses.count], dtype=torch.float32)
        ...
        sync_tensor = torch.tensor([top1.sum, top1.count, top5.sum, top5.count, losses.sum, losses.count], dtype=torch.float32).cuda()
...
def validate(val_loader, model, criterion, opt, epoch='N/A'):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            if i % opt.print_freq == 0:
                print('<rank {}> Eval epoch={}, i={}, loss={:4g}, acc1={:4g}%, acc5={:4g}%'.format(
                    opt.rank, epoch, i, float(loss), float(acc1[0]), float(acc5[0])))
        pass  # end for
    pass  # end with
    ...
    logger.info(' * Validate Acc@1 {:.3f} Acc@5 {:.3f}, n_val={}'.format(top1_acc_avg, top5_acc_avg, total_val_count))
    # if opt.rank == 0:    
...
def main(opt, argv):
    ...
    writer = None
    if opt.rank in [-1, 0]:
        print(f'Start Tensorboard with "tensorboard --logdir=runs -port", view at http://localhost:6006/')
    ...
    if opt.rank == 0:
        ...
        logger = get_logger(log_filename=None, level=logging.ERROR)
    ...
    logger.info('loading student_model:')
    ...
    logger.info('loading teacher_model:')
    logger.info(str(teacher_model))

    logger.info('create teacher-student model')
    ...
    if opt.gpu is not None:
        ...
        print('rank={}, using GPU {}'.format(opt.rank, opt.gpu))
